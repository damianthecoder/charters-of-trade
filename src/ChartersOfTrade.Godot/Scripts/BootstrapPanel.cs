...
public partial class BootstrapPanel : Control
{
    ...
    private IReadOnlyList<RouteContractOption> _visibleContracts = [];
    ...
    private void AppendRouteInspector(TradeRoute route)
    {
        ...
        var contractCount = ContractOptionsFromSnapshot(_snapshot).Count(contract => contract.RouteId == route.Id);
        ...
        _inspector.AppendText(contractCount > 0
            ...
            : "Contract options: pending bridge data\n");
    ...
    private void UpdateContractControl()
    {
        ...
        var bridgeHasContracts = HasContractBridgeSurface();
        var selectedContractId = SelectedContractIdFromSnapshot(_snapshot);
        ...
        _visibleContracts = ContractOptionsFromSnapshot(_snapshot)
        ...
        {
            _contractOptions.AddItem(bridgeHasContracts ? "No matching contracts" : "Contracts unavailable");
            _contractOptions.Disabled = true;
            _contractActionButton.Disabled = true;
            _contractSummary.Text = bridgeHasContracts
                ? ContractScopeText("No available route contract for")
                : "Route contract controls are ready; bridge data is not available in this checkout.";
            return;
        }
        ...
        for (var i = 0; i < _visibleContracts.Count; i++)
        {
            ...
            _contractOptions.AddItem($"{contract.ResourceId} {contract.FromNode}->{contract.ToNode} net {contract.ExpectedNet:0.00}", i);
        ...
        _contractActionButton.Disabled = !HasRouteContractSelector();
    ...
    private void SelectVisibleRouteContract()
    {
        ...
        var method = _session.GetType().GetMethod("SelectRouteContract", [typeof(string)]);
        if (method is null)
        {
            UpdateContractControl();
            return;
        ...
        var result = method.Invoke(_session, [_visibleContracts[index].Id]);
        if (result is PrototypeSnapshot snapshot)
        {
            _snapshot = snapshot;
        }
        else if (_session.GetType().GetProperty("Current")?.GetValue(_session) is PrototypeSnapshot current)
        {
            _snapshot = current;
        }
    ...
    private bool ContractAppliesToCurrentSelection(RouteContractOption contract)
    {
        if (_selectedRouteId is not null)
        {
    ...
    private bool HasContractBridgeSurface()
    {
        return _snapshot?.GetType().GetProperty("AvailableContracts") is not null && HasRouteContractSelector();
    }

    private bool HasRouteContractSelector()
    {
        return _session?.GetType().GetMethod("SelectRouteContract", [typeof(string)]) is not null;
    }

    private static IReadOnlyList<RouteContractOption> ContractOptionsFromSnapshot(PrototypeSnapshot snapshot)
    {
        var value = snapshot.GetType().GetProperty("AvailableContracts")?.GetValue(snapshot);
        if (value is not IEnumerable contracts || value is string)
        {
            return [];
        }

        var options = new List<RouteContractOption>();
        foreach (var contract in contracts)
        {
            ...
            var id = ReadString(contract, "Id");
            var routeId = ReadString(contract, "RouteId");
            var from = ReadString(contract, "FromNode");
            var to = ReadString(contract, "ToNode");
            var resourceId = ReadString(contract, "ResourceId");
            ...
            options.Add(new RouteContractOption(
                id,
                routeId,
                from,
                to,
                resourceId,
                ReadDecimal(contract, "ExpectedRevenue"),
                ReadDecimal(contract, "TransportCost"),
                ReadDecimal(contract, "ExpectedNet"),
                ReadInt(contract, "CapacityPerDay")));
        }

        return options;
    }

    private static string? SelectedContractIdFromSnapshot(PrototypeSnapshot snapshot)
    {
        return ReadString(snapshot, "SelectedContractId");
    }

    private static string ContractSummary(RouteContractOption contract)
    {
        return $"{contract.ResourceId} on {contract.RouteId}, revenue {contract.ExpectedRevenue:0.00}, cost {contract.TransportCost:0.00}, net {contract.ExpectedNet:+0.00;-0.00;0.00}, cap {contract.CapacityPerDay}/day";
    }
    ...
    {
        return ReadValue(source, name) switch
        {
            null => null,
    ...
    private static decimal ReadDecimal(object source, string name)
    {
    ...
    private sealed record RouteContractOption(
        string Id,
        string RouteId,
        string? FromNode,
        string? ToNode,
