...
- `ChartersOfTrade.Godot`: review fix preserves the bridge's shipment-priority contract ordering in dropdowns and inspectors, and ranks top pressure by shipment priority before scarcity.
...
- Review found P1: Godot contract UI re-sorted contracts by net value and could contradict the bridge's shipment-priority ordering. Fixed by preserving snapshot order after filtering.
- Review found P3: top pressure summaries ranked by scarcity only. Fixed by ranking shipment priority before scarcity.
