...
- Stage 5 now consumes Stage 3 production opportunities and Stage 4 route-operation candidates as derived deterministic NPC pressure. It replaces the old raw-stock AI move path without adding save state.
...
5. Review and merge Stage 5 deterministic NPC pressure against Stage 3/4 candidate surfaces.
...
- Stage 5 verification after delegated review fixes: `tools/build.ps1` passed with 0 warnings and 0 errors; `tools/test.ps1` passed with 47/47 console tests plus `INTERACTION_SMOKE PASS` and `VISUAL_SMOKE PASS`; `tools/benchmark.ps1` passed 25/25 playable seeds with average unmet demand 0.7115, median time-to-profit 1.0, and bankruptcy 0/25; `tools/visual-qa.ps1` passed with 18 captures in `artifacts/godot-visual-qa/visual-qa-20260430-195421`.
...
Proceed to delegated review for Stage 5, then merge if no blockers. Keep persistent NPC company state and full multi-route operations deferred until save-format ADR decisions.
