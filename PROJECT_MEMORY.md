...
- Production-chain opportunities are currently read-only bridge metadata/UI; destination margin and route id are demand hints until Stage 4 route operations can realize them. Adding production focus, build orders, or recurring production commands would be gameplay state and needs a save-format decision.
